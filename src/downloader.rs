...
pub const DEFAULT_DOWNLOAD_DIRECTORY: &str = "./sankaku-downloads/images";
pub const DEFAULT_CACHE_DIRECTORY: &str = "./sankaku-downloads/cache";

async fn request_file_bytes(image: &Image) -> Result<bytes::Bytes, Error> {
...
pub async fn save_file_to_disk(image: &Image) -> Result<(), Error> {
	let file_path = format!("{}/{}", DEFAULT_DOWNLOAD_DIRECTORY, image.file_name);

	if !Path::new(&file_path).exists() {
		let mut file = File::create(file_path).expect("Failed to create file.");
		let data = request_file_bytes(image).await.expect("Failed to download image data.");
